 ...
 */
function table<T>(ctx: Layout, columns: readonly Column<T>[], rows: readonly T[]): void {
  const { doc } = ctx;
  const rowHeight = 15;
 ...
 */
function stampFooters(doc: Doc, report: AnalyticsReport, title: string): void {
  ...
  for (let i = 0; i < range.count; i++) {
    ...
    doc
      ...
      .fontSize(7)
      .fillColor(COLOR.faint)
      .text(`${title} · generated ${fmtDateTime(report.meta.generatedAt)}`, MARGIN, FOOTER_TOP + 14, {
